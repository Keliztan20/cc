...
def get_users_with_role_name(role_name: str = Query(...), db: Session = Depends(get_db)):
    user_role = db.query(RoleModel).filter(RoleModel.role_name.ilike(f"%{role_name}%")).all()
    if not user_role:
...
def delete_role(role_id: int = Query(...), db: Session = Depends(get_db)):
