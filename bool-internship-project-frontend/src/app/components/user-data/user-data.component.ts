...
export class UserDataComponent implements OnInit {
  ...
  nextGroup(): void {
    if (this.currentPage + 1 <= this.totalPages) {
      this.changePage(this.currentPage + 1); // Set to the first page of the next group
    }
  }
  ...
  previousGroup(): void {
    if (this.currentPage - 1 > 0) {
      this.changePage(this.currentPage - 1); // Set to the first page of the previous group
    }
  }
  ...
  addUser(firstName: string, lastName: string, role_name: string): void {
    if (!role_name || !firstName.trim() || !lastName.trim()) {
      alert('Some of input data required to add user');
      return;
    }
    // Find the role by matching role_name
    const selectedRole = this.roles.find(role => role.role_name.toLowerCase() === role_name.toLowerCase());
    ...
    // Extract role_id_FK
    const role_id_FK = selectedRole.role_id;

    let queryParams = [];
    if (firstName) queryParams.push(`first_name=${encodeURIComponent(firstName)}`);
    if (lastName) queryParams.push(`last_name=${encodeURIComponent(lastName)}`);
    if (role_name) queryParams.push(`role_id_FK=${encodeURIComponent(role_id_FK)}`);
  ...
  updateUser(firstName: string, lastName: string, role_name: string): void {
    // Find the role by matching role_name
    const selectedRole = this.roles.find(role => role.role_name.toLowerCase() === role_name.toLowerCase());
    ...
    if (firstName) queryParams.push(`first_name=${encodeURIComponent(firstName)}`);
    if (lastName) queryParams.push(`last_name=${encodeURIComponent(lastName)}`);
    if (role_id) queryParams.push(`role_id_FK=${encodeURIComponent(role_id)}`);
